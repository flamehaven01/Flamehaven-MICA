...
Step 3: Classify Situation
  ...
  CASE B: No mica.yaml + memory/*.mica.*.json found
    ...
    → Warning: "mica.yaml not found. This archive is pre-v0.1.9 (unmigrated)."
    → Load archive JSON directly (apply § 4 layer processing rules)
    → Recommend migration: see MICA_v0.1.9_MIGRATION_GUIDE.md
...
```json
{
  ...
  "operation_meta": {
    ...
    "session_count": 0,
    "bootstrap_note": "MICA v0.1.9 initial bootstrap. design_invariants and provenance_registry intentionally empty — populate during the first real maintenance session."
  },
  "invocation_protocol": {
...
|------|---------|---------|
| [YYYY-MM-DD] | v1.0.0 | Initial MICA v0.1.9 bootstrap |
```
...
```
[MICA v0.1.9 INSERTION COMPLETE]
...
PCT-005 INFO  archive mica_spec = 0.1.9
PCT-006 PASS  mica_spec aligned
PCT-007 PASS  package complete

Status: MICA v0.1.9 installed and validated. Memory layer is live.
...
MUST-NOT-10  Interpret legacy v0.1.8.x archives using v0.1.9 rules without migration.
             (See: MICA_v0.1.9_MIGRATION_GUIDE.md)
```
...
mica.yaml full field reference:
  → MICA_v0.1.9_COMPOSITION_CONTRACT.md
...
archive v0.1.9 changes (patch-as-spec):
  → mica-v0.1.9-archive-changes.schema.json

PCT check definitions (full):
  → mica-v0.1.9-self-test-expansion.schema.json

v0.1.8.1 → v0.1.9 migration:
  → MICA_v0.1.9_MIGRATION_GUIDE.md

mica.yaml reference examples (both modes):
  → MICA_v0.1.9_EXAMPLES.md
```
...
→ Notify user: "v0.1.8.x structure detected. Add mica.yaml to upgrade to v0.1.9?"
```
