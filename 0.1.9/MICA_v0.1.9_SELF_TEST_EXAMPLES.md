# MICA v0.1.9 self_test_policy — Package Completeness Examples
...
"self_test_policy": {
  ...
  "note": "v0.1.9 package completeness checks. Run in order — hard-fail checks stop session before softer checks execute.",
  "checks": [
...

    {
      ...
      "severity": "info",
      "on_fail": "INFO: archive does not have mica_spec field (legacy-valid, COMPAT-001). Add mica_spec: '0.1.9' on next maintenance version bump.",
...
"self_test_policy": {
  ...
  "on_failure": "require_acknowledgment",
  "note": "v0.1.9 package completeness checks for protocol_evolution mode. lessons layer is required. exemplars is optional (required: false in mica.yaml).",
  "checks": [
...

    {
      ...
      "check_type": "mica_spec_present",
      "severity": "info",
      "on_fail": "INFO: CCGE BASELINE archive has mica_schema_version='baseline-draft' (template-valid, COMPAT-004). When instantiating for a target repo, add mica_spec: '0.1.9'.",
...
```
Order:
  PCT-001 ~ PCT-007  ← v0.1.9 new (package completeness)
